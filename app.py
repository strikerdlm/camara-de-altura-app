...
class RegistroApp:
    ...
    def setup_styles(self):
        ...
        # Use the style object created in __init__
        # style = ttkb.Style() # Remove this line
    ...
    def setup_autosave(self):
        ...
        # We could still allow config override, but default/minimum should be 10
        # Let's force 10 seconds for now based on the rule.
        ...
        # Original code using config:
        # autosave_interval = self.config.get_setting("autosave_interval", 60) * 1000  # Convert to milliseconds
        ...
        # Use a dedicated attribute to store the job ID for potential cancellation on exit
        self._autosave_job = self.root.after(autosave_interval_ms, autosave)
...
if __name__ == "__main__":
    # Add time import needed for chronometer
    import time 
    # Create directories if they don't exist
    os.makedirs(os.path.join(os.path.dirname(__file__), "assets"), exist_ok=True)
    os.makedirs(os.path.join(os.path.dirname(__file__), "data"), exist_ok=True)
    
    # --- Minimal Asset Check (Icon Only) --- 
    icon_asset_path = os.path.join(os.path.dirname(__file__), "assets", "icon.ico")
    if not os.path.exists(icon_asset_path):
        try:
            import create_placeholder_assets
            create_placeholder_assets.create_icon()
        except ImportError:
            print("Warning: create_placeholder_assets.py not found. Cannot create default icon.")
        except Exception as e:
            print(f"Error creating placeholder icon: {e}")
    
    # --- Launch RegistroApp Directly --- 
    print("Launching main application directly...")
    # Create main application window with the desired theme
    main_root = ttkb.Window(themename="litera") # Use the desired theme for the main app
    ...
    try:
         ...
         print(f"Could not set initial window geometry: {e}")
         # Fallback size
         main_root.geometry('1200x800') 

    # Create the main application instance
    app = RegistroApp(main_root)
    print("Main application UI created.")
    
    # Start the Tkinter main loop for the main application
    main_root.mainloop() # This should be the ONLY mainloop call now 