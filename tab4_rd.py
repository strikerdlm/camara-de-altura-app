...
class RDTab(ttkb.Frame):
    ...
    def save_data(self):
        """Save RD data to the data manager's session structure."""
        # Collect data from form fields
        rd_data = {}
        for key, var in self.variables.items():
            rd_data[key] = var.get()

        # Save to data manager - general structure
    ...
    def clear_form(self):
        ...
        if messagebox.askyesno(
            ...
            # Clear all variables
            for var_name, var in self.variables.items():
                # Reset to empty string, except for evento_adverso which gets default 'No'
                if var_name in ['evento_adverso_rd1', 'evento_adverso_rd2']:
                    var.set('No')
                else:
                    var.set('')
