...
class ReportesTab(ttkb.Frame):
    ...
    def load_data(self):
        """Clear the preview tree when new data is loaded (e.g., after loading a training)."""
        try:
            # Clear existing items in the preview tree
            for item in self.preview_tree.get_children():
                self.preview_tree.delete(item)
        except Exception as e:
            print(f"Error clearing preview tree in load_data: {e}")
    ...
    def clear_form(self, confirm=True):
        ...
        if confirm:
            if not messagebox.askyesno(
                "Confirmar",
                "¿Está seguro de limpiar los campos de reportes?\n\nEsta acción borrará los datos en esta pestaña."
            ):
                return
        ...
        try:
            ...
            print(f"Error clearing preview tree in clear_form: {e}")
