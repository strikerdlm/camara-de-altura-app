...
class SintomasTab(ttkb.Frame):
    ...
    def __init__(self, parent, data_manager):
        ...
        self.symptom_options = [
            "Ansiedad", "Cansancio fisico", "Cansancio mental", "Dolor de cabeza",
    ...
    def load_data(self):
        ...
        # Load symptoms and update display labels
        for student_id, symptoms in self.student_symptoms.items():
            self.update_symptom_summary(student_id)

    def save_data(self):
        """Save student symptoms to data manager."""
        self.data_manager.current_data['student_symptoms'] = self.student_symptoms
        
        try:
            self.data_manager.save_data()
            print("Student symptoms saved.")
        except Exception as e:
            print(f"Error saving student symptoms: {e}")

    def clear_all_symptoms(self):
        """Clear all recorded symptoms for all students."""
        confirm = messagebox.askyesno(
            "Confirmar Limpieza",
            "¿Está seguro que desea limpiar TODOS los síntomas registrados?",
            icon="warning",
            parent=self
        )
        if not confirm:
        ...
        # Clear all student symptoms
        for student_id in self.student_symptoms.keys():
            self.student_symptoms[student_id] = []
            self.update_symptom_summary(student_id)
        
        self.save_data()  # Save the cleared state
        print("Todos los síntomas han sido limpiados.") 