...
class VueloTab(ttkb.Frame):
    ...
    def save_data(self):
        ...
        # Generate the numero_entrenamiento if not already set
        ...
        # Get session ID for saving all session data
        ...
        if session_id:
            ...
            # Get existing session data to preserve other tabs' data
            session_data = self.data_manager.current_data['sessions_data'][session_id]
            
            # Save vuelo data to the session
            session_data['vuelo'] = vuelo_data
            
            # Save other tab data to the session if available
            # RD data
            if 'rd' in self.data_manager.current_data:
                session_data['rd'] = self.data_manager.current_data['rd']
                
            # Alumnos data
            if 'alumnos' in self.data_manager.current_data:
                session_data['alumnos'] = self.data_manager.current_data['alumnos']
                
            # Reactions data
            if 'reactions_data' in self.data_manager.current_data:
                session_data['reactions_data'] = self.data_manager.current_data['reactions_data']
                
            # Student symptoms data
            if 'student_symptoms' in self.data_manager.current_data:
                session_data['student_symptoms'] = self.data_manager.current_data['student_symptoms']
            
            # Save the session data back to the main data structure
            self.data_manager.current_data['sessions_data'][session_id] = session_data
    ...
    def save_as_new_training(self):
        ...
        # Generate the numero_entrenamiento (session ID)
        year_suffix = datetime.now().strftime("%y")
    ...
    def load_training(self, training_id):
        ...
        # Get number for session if available
        session_id = training_data.get('numero_entrenamiento', '')
        if not session_id and 'vuelo_del_ano' in training_data:
            # Generate the numero_entrenamiento from vuelo_del_ano
            vuelo_del_ano = training_data['vuelo_del_ano']
            # Try to extract year from date
            try:
                date_str = training_data.get('fecha', '')
                if date_str:
                    # Try DD-MM-YYYY format
                    try:
                        ...
                        # Try other formats or use current year
                        year_suffix = datetime.now().strftime("%y")
                else:
                    year_suffix = datetime.now().strftime("%y")
                session_id = f"{vuelo_del_ano}-{year_suffix}"
                training_data['numero_entrenamiento'] = session_id
            except Exception as e:
                print(f"Error generating session_id: {e}")
        ...
        # Initialize sessions_data structure if needed
        if 'sessions_data' not in self.data_manager.current_data:
            self.data_manager.current_data['sessions_data'] = {}
            
        # Create or update session entry using the session_id
        if session_id:
            # Initialize the session data
            self.data_manager.current_data['sessions_data'][session_id] = {}
            session_data = self.data_manager.current_data['sessions_data'][session_id]
            
            # Always include vuelo data
            session_data['vuelo'] = training_data.copy()
            
            # Check if we have a current_session in the archived data
            if archived_data and 'current_session' in archived_data:
                # Use the specific session data that was saved
                for key, value in archived_data['current_session'].items():
                    session_data[key] = value
                    # Also update the main data structure for backward compatibility
                    self.data_manager.current_data[key] = value
            elif archived_data:
                # If no specific session data, try to gather data from individual tabs
                for key in archived_data:
                    if key != 'vuelo' and key != 'sessions_data':  
                        session_data[key] = archived_data[key]
                        # Also update the main data structure for backward compatibility
        ...
        try:
            ...
            # Notify parent application to refresh all tabs
            try:
                # Use the direct reference to main_app if available
                if self.main_app and hasattr(self.main_app, 'refresh_all_tabs'):
                    self.main_app.refresh_all_tabs()
                    print("All tabs refreshed with new data")
                else:
                    print("MainApp reference not available or missing refresh_all_tabs method")
            except Exception as e:
